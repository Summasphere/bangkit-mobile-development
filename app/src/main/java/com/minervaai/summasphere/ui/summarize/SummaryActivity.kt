    ...
    private fun summarize(textInput: String, urlInput: String, model: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                ...
                runOnUiThread {
                    Toast.makeText(this@SummaryActivity, "Request failed: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                runOnUiThread {
                    if (code == 200 && status == "success") {
                        ...
                        Toast.makeText(this@SummaryActivity, "Text summarized successfully", Toast.LENGTH_SHORT).show()

                        // Start ResultSummarizerActivity and pass the summary
                        val intent = Intent(this@SummaryActivity, ResultSummarizerActivity::class.java).apply {
                            putExtra("SUMMARY_RESULT", summary)
                        }
                        startActivity(intent)
                    } else {
                        Toast.makeText(this@SummaryActivity, message ?: "Failed to summarize text", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        })
